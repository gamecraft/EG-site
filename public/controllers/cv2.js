(function(){
    global.modules.CV2 = function(memberId){
        ...
        var renderItems = function(target, items, img) {
            ...
            for(var i = 0; i<items.length; i++) {
                ...
                ox +=  cardImg.width()+10;
                if(ox+cardImg.width()>1000) {
                    ox = 150;
                    if(i != items.length-2)
                        advanceCurrentOffsetY(target,imgHeight+spacey,imgHeight);
                    else
                        advanceCurrentOffsetY(target,imgHeight+spacey);
                }
            }
            if(items.length % 3 != 0)
                advanceCurrentOffsetY(target,imgHeight+spacey+30,imgHeight);
        }
